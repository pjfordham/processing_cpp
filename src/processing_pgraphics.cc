...
public:
   ...
   void ortho(float left, float right, float bottom, float top, float near, float far) {
      ...
      float pixel_size_x = 1.0f / width;
      float pixel_size_y = 1.0f / height;

      // Modify the projection matrix to shift rendering right and down by 1/2 pixel
      projection[0][2] -= pixel_size_x; // Shift right
      projection[1][2] -= pixel_size_y; // Shift down
